...
function createPasskeyStore({
  ...
  const db = () => new Promise((resolve) => setImmediate(() => resolve(model())));
    ...
    if (!ready) {
      ready = (async () => {
        ...
        (err) => {
          ...
          failed = {
            at: now(),
            error: new StoreError(503, 'passkey_storage_error', `The passkey table ${table} could not be created: ${err.message}`),
          };
          throw failed.error;
        },
      );
  ...
  async function state({ fresh = false } = {}) {
    try {
      ...
      return envOnly(err instanceof StoreError ? err : new StoreError(503, 'passkey_storage_error', `Passkeys could not be read: ${err.message}`));
    }
  }
