...
`PASSKEY_ENABLED` (true when the passkey table had a passkey at this instance's last read, which is at most 30 seconds old and never delays the page; before the first read, when `PASSKEYS` holds a valid entry) and
...
dates ISO 8601 strings. Reads are cached for 30 seconds per instance and
dropped on every write; sign-in always reads the table, so a removed passkey
stops working everywhere at once. Other Waline storage (PostgreSQL, MongoDB,
LeanCloud, …), or a table that cannot be created, leaves passkeys read-only
from `PASSKEYS`, and the Profile page says why.
