...
describe('passkey store on SQLite', () => {
  ...
  test('register, list, rename, touch and remove with a tombstone', async () => {
    ...
    await store.rename('NeW_1', 'Work \u0000  phone');
    assert.equal((await store.state()).passkeys.find((p) => p.id === 'NeW_1').name, 'Work phone');
  ...
  test('a table that exists is used even when CREATE TABLE is not allowed', async () => {
    const model = sqliteModel();
    const errors = [];

    await createPasskeyStore({ env: {}, type: 'sqlite', model }).add({ id: 'ExIsTs', publicKey: 'QUJD', userId: '1' });

    const denied = () => {
      const instance = model();
      const execute = instance.execute.bind(instance);

      instance.execute = (sql) => (/^CREATE/u.test(String(sql)) ? Promise.reject(new Error('CREATE command denied')) : execute(sql));

      return instance;
    };
    const store = createPasskeyStore({ env: {}, type: 'sqlite', model: denied, logger: { error: (...a) => errors.push(a) } });
    const state = await store.state();

    assert.equal(state.storage, 'database');
    assert.deepEqual(state.passkeys.map((p) => p.id), ['ExIsTs']);
    await store.rename('ExIsTs', 'Still works');
    assert.equal((await store.find('ExIsTs')).entry.name, 'Still works');
    assert.equal(errors.length, 1);
  });

  test('a model that throws is a read-only fallback, not a crash', async () => {
    let calls = 0;
    const flaky = () => {
      calls += 1;
      if (calls > 1) throw new Error('adapter gone');

      return { tableName: 'wl_Passkeys' };
    };
    const store = createPasskeyStore({ env: { PASSKEYS: envValue }, type: 'mysql', model: flaky, logger: { error() {} } });
    const state = await store.state();

    assert.equal(state.storage, 'env');
    assert.match(state.error.message, /adapter gone/u);
    assert.deepEqual(state.passkeys.map((p) => p.id), ['EnV-_1']);
    await store.touch('EnV-_1');
  });
});
